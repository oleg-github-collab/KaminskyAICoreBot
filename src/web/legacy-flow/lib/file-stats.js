...
class FileStats {
    ...
    estimateCost(file) {
        ...
        // UI-only estimate. Authoritative pricing comes from /api/projects/:id/pricing.
        // Simple text/docx: €0.91 per 1800 chars with configured glossary.
        // Complex/OCR formats: €1.35 per 1800 chars/page-equivalent.

        let cost = 0;

        if (file.mime_type.includes('text/') || file.mime_type.includes('word')) {
            cost = Math.ceil(charCount / 1800) * 0.91;
        } else {
            cost = pageCount * 1.35;
        }
