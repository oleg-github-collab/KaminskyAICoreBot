...
const InstructionsView = {
    ...
    async loadInstructions(projectId) {
        try {
            ...
            // Fall back to defaults, but make sure no skeleton spins forever —
            // render an explicit error state in any container we can't fill.
            ...
            try {
                ...
                console.error('Failed to render instructions fallback:', renderErr);
                const errHtml = '<div class="error-state" style="padding:16px">Не вдалося завантажити інструкції. Спробуйте оновити.</div>';
                const lib = document.getElementById('library-list');
                const active = document.getElementById('active-instructions');
                const preview = document.getElementById('prompt-preview');
                if (lib) lib.innerHTML = errHtml;
                if (active) active.innerHTML = errHtml;
                if (preview) preview.innerHTML = errHtml;
            }
        }
    },
    ...
    async processUploadedFile(file) {
        ...
        try {
            ...
            const uploadResp = await fetch(API.base + '/projects/' + pid + '/files', {
                method: 'POST',
