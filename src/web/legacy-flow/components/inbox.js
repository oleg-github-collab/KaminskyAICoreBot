...
const InboxView = {
    ...
    async loadItems(projectId) {
        ...
        // The inbox backend route is not implemented yet — degrade gracefully if the
        // API method is missing or fails instead of crashing the view.
        if (typeof API === 'undefined' || typeof API.getInboxItems !== 'function') {
            this.items = [];
            this.updateCounts();
            if (list) {
                list.innerHTML = `
                    <div class="empty-state">
                        ...
                        <div class="empty-text">Розділ недоступний</div>
                    </div>
                `;
            }
            return;
        ...
        try {
            ...
            if (list) {
                list.innerHTML = `
                    <div class="empty-state">
                        <div class="empty-icon">📭</div>
                        <div class="empty-text">Поки що немає повідомлень</div>
                    </div>
                `;
            }
        }
    ...
    downloadFile(fileId) {
        window.open(`${API.base}/files/${fileId}/download`, '_blank');
    },
