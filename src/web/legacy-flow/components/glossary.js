const GlossaryView = {
    ...
    async render(c, project) {
        if (!project) {
            c.innerHTML = '<div class="empty-state"><div class="empty-state-icon">' + Icons.wrap('glossary', 48) + '</div><p class="empty-state-title">Оберіть проєкт</p><button class="btn btn-primary" style="margin-top:12px" onclick="App.backToProjects()">До проєктів</button></div>';
            return;
        }
        c.innerHTML = `
    ...
    // Disable/enable all action buttons + inputs in a term row during async work.
    _setRowBusy(termId, busy) {
        const row = document.querySelector('.term-row[data-id="' + termId + '"]');
        if (!row) return;
        row.querySelectorAll('button, input').forEach(el => { el.disabled = busy; });
    },
    ...
    async importFile(pid, input) {
        ...
        try {
            ...
            App.modalConfirm(
                ...
                async () => {
                    // Guard against re-entry (modal closes before this resolves).
                    if (this._importing) return;
                    this._importing = true;
                    App.toast('Імпорт глосарію...', 'info');
                    try {
